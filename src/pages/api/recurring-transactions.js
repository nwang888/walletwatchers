...
async function postTransaction(transaction){
    ...
    try{
      ...
      VALUES (?, ?, ?, ?, ?)
      `,[transaction.transaction_id, transaction.primary_category, transaction.detailed_category, transaction.merchant_name, transaction.transaction_amount]);
    }
    catch(e){
...
export default async function TransactionHandler(req, res) {
  ...
  if (req.method == "GET") {
    try{
      ...
      if(payload.length == 0){
        console.log(payload);
        return res.status(200).json({message: "no recurring transactions set"});
      }
      else{
  ...
  if (req.method === "POST") {
      const {primary_category, detailed_category, merchant_name, transaction_amount} = req.body;
      
      //the id for the recurring transaction
      console.log(req.body);
      const transaction_id = Math.floor(Math.random() * 10000000)

      // Post the recurring transaction to the database
      try {
          postTransaction({transaction_id, primary_category, detailed_category, merchant_name, transaction_amount});
          res.status(200).json({ message: "Recurring transaction set successfully!" });
      } catch (error) {
          res.status(500).json({ message: "Failed to set recurring transaction.", error: error.message });
      }
  }
}
